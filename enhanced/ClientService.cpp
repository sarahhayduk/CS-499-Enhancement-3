 ...
 * hardcoded password vulnerability. Returns 1 for success, 2 for invalid creditial,
 * and -1 for system failure.
 * -------------------------------------------------------------------------------*/
int CheckUserPermissionAccess() {
    ...
    // If fail to load hash and salt, treat as system error and stop authentication
    if (!GetPassword(storedHash, storedSalt)) {
        cout << "[Error] Failed to process request." << endl;
        return -1; // treat as system failure
    }
...
int main()
{
    ...
    while (answer != 1) {
        ...
        // If system error occurs, stop program and close database
        if (answer == -1) {
            cout << "[Error] Authentication system error. Exiting." << endl;
            CloseDatabase();
            return 1;
        }
